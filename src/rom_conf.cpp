 ...
 *
 */
void conf_save(void) { EEPROM.put(0, config); }

void conf_getAppEui(uint8_t *buf) { memcpy(buf, &config.APP_EUI, 8); }

void conf_getDevEui(uint8_t *buf) { memcpy(buf, &config.DEV_EUI, 8); }

void conf_getAppKey(uint8_t *buf) { memcpy(buf, &config.APP_KEY, 16); }

uint8_t conf_getUseTTNFairUsePolicy() { return config.USE_TTN_FAIR_USE_POLICY; }
...
uint16_t conf_getMeasurementInterval() {
  // Set bounds for rom measurement interval, worst-case scenario the device
  // keeps functioning
...
uint16_t versionToUint16(version v) {
  uint16_t dat = ((v.proto & 0x01) << 15) | ((v.major & 0x1F) << 10) |
                 ((v.minor & 0x1F) << 5) | ((v.patch & 0x1F) << 0);
  return dat;
}