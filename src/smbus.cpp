...
void twi_stop(void) { TWCR = _BV(TWSTO) | _BV(TWINT) | _BV(TWEN); }
...
error_t smbus_blockRead(uint8_t addr, uint8_t cmd, uint8_t *rx_buf,
                        uint8_t *rx_length) {
  error_t status = ERR_NONE;
...
error_t smbus_blockWrite(uint8_t addr, uint8_t cmd, uint8_t *tx_buf,
                         uint8_t tx_length) {
  error_t status = ERR_NONE;
  // Start transaction and write command
  if ((status = twi_start(addr, TW_WRITE)) != ERR_NONE) {
